...
{
    public class BackTriple : BarrageBase
    {
        private const float NEXT_OFFSET = 0.5f;
        private const float FIRE_OFFSET = -0.5f;

        public BackTriple(PoolService poolService, ProjectileConfig config, UnitTag damageTag, EventBus eventBus)
        ...
        public override IEnumerator StartBarrage(Vector2 firePos)
        {
            for(int i = 0; i < 3; i++)
            {
                ...
                SetPosAndDir(nextPos, Vector2.down);
            }
        }
    }
}
